...
class BotDb {
  ...
  insertGameLog(userId, gameName, startTime, endTime, playTime) {
    return new Promise((resolve, reject) => {
      this.db.serialize(() => {
        this.db.exec('BEGIN TRANSACTION');

        const stmt = this.db.prepare('INSERT INTO game_log(user_id, game_name, start_time, end_time, play_time) VALUES(?, ?, ?, ?, ?)');
